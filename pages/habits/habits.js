...
async function loadHabits() {
  ...
  try {
    ...
    _state.habits = habitsArr.map(h => {
      ...
      const habitLogs = logsArr.filter(l => l.habitId === h.id);
      return {
        ...h,
        _doneToday: logsToday.has(h.id),
        _isToday: isToday,
        _streak: calcularStreak(h, habitLogs),
        _last7: calcularLast7(h, habitLogs)
      };
    });
  } catch (err) {
...
function calcularStreak(habit, logs) {
...
function calcularLast7(habit, logs) {
  const dowMap = ['sun','mon','tue','wed','thu','fri','sat'];
  const labelMap = { mon:'Seg', tue:'Ter', wed:'Qua', thu:'Qui', fri:'Sex', sat:'Sab', sun:'Dom' };
  const daysOfWeek = Array.isArray(habit.daysOfWeek) ? habit.daysOfWeek : [];
  const doneDates = new Set((logs || []).filter(l => l.completed).map(l => l.date));
  const result = [];
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    const applicable = daysOfWeek.length === 0 || daysOfWeek.includes(dow);
    result.push({
      ymd: ymd,
      label: labelMap[dow],
...
function buildStreakSection(habit) {
  const section = document.createElement('div');
  section.className = 'habit-item__streak';

  const left = document.createElement('div');
  left.className = 'habit-item__streak-info';

  const fire = document.createElement('span');
  fire.className = 'habit-item__streak-fire';
  fire.textContent = '\uD83D\uDD25';
  left.appendChild(fire);

  const num = document.createElement('span');
  num.className = 'habit-item__streak-num';
  if (habit._streak > 0) {
    ...
    num.classList.add('habit-item__streak-num--empty');
  }
...
function renderHabitItem(habit) {
  ...
  item.appendChild(buildStreakSection(habit));
